...
bool MyApp::OnInit()
{
	...
	//初始化RTServerManager
	new RTServerManager;
	if (sRTServerManager.Init() == false)
	{
		LOG("初始化RTServerManager 失败")
		return false;
	}
	else
	{
		LOG("初始化RTServerManager 成功")
	}
	...
	//显示Frame
	g_pMainFrame = new MyFrame((wxWindow*)g_pLoginDialog, w_RemoteTool);
	...
	//开启处理消息线程
	sRTServerManager.HandleMessages();
...
{
	//清理RTServerManager
	if(sRTServerManager.getSingletonPtr() != NULL)
	{
		delete sRTServerManager.getSingletonPtr();
	}
	return 0;
}
...
{	
	...
	sRTServerManager.ConnectRTServer();
}
                                    ...
                                    wxPoint(0, 0), wxSize(980, 600), wxDEFAULT_FRAME_STYLE |     wxNO_FULL_REPAINT_ON_RESIZE|wxCLIP_CHILDREN|wxTAB_TRAVERSAL)
{
	...
	open_close_page_		= new OpenAndCloseServerPage(book_, w_OpenCloseServer);
	...
	//设置PageAssist指针
	sRTServerManager.SetPageAssistPtr(page_assist_);
	gm_mgr_page_->SetPageAssist(page_assist_);

	//设置各个页面的Update回调函数指针
	page_assist_->SetOpenAndCloseServerPageFunc(open_close_page_, &OpenAndCloseServerPage::update_ui);
}
...
void MyFrame::ShowPage(bool bShow_1, bool bShow_2, bool bShow_3, bool bShow_4)
{
	...
	OpenAndCloseServerPage* pPage = (OpenAndCloseServerPage*)(book_->GetPage(0));

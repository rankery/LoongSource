...
--与珍肴灵壤对话，珍肴土
function ZhenYaoTu_OnTalk(MapID, InstanceID, TargetID, TargetTypeID, RoleID, TalkIndex)
...
--1.5新增内容/////////////////////////////////////////////////down,所有材料▲▲▲▲▲▲▲▲
	...
	if farmer_level > 120 then
		...
		if TalkIndex == -1 then
			if seedowner ~= 0 then
				local MsgID = msg.BeginMsgEvent()
				msg.AddMsgEvent(MsgID, 20, 1328)	-- "###(玩家名)所种的###(植物名),灵种刚刚开始发芽，多多浇水可促进其生长！"
				...
				local MsgID = msg.BeginMsgEvent()
				msg.AddMsgEvent(MsgID, 20, 1331)	-- "珍肴灵壤刚刚开始发芽，多多浇水可促进其生长！\n它还没有主人，只要您浇过一次水即为它的新主人"
				msg.AddMsgEvent(MsgID, 21, 4)	-- 选项一
				msg.AddMsgEvent(MsgID, 1, 1302)	-- "★浇水"
				msg.AddMsgEvent(MsgID, 21, 5)	-- 选项二
				msg.AddMsgEvent(MsgID, 1, 1303)	-- "离开"
	...
	if 	HaveDelete_growthitem == 1 then
		...
		if growtime == 5 then
			local x, y, z = unit.GetPosition(MapID, InstanceID, TargetID)
			local creatureID = map.MapCreateCreature(MapID, InstanceID, 4900412, x, y, z)
			if seedowner == 0 then
			seedowner = RoleID
			end
...
aux.RegisterCreatureEvent(4900411, 7, "ZhenYaoTu_OnTalk")
...
--与百味灵树对话，百味树
function ZhenYaoShu_OnTalk(MapID, InstanceID, TargetID, TargetTypeID, RoleID, TalkIndex)
	local seedowner = cre.GetCreatureScriptData(MapID, InstanceID, TargetID, 1, 1)--种植者ID
	local growtime = cre.GetCreatureScriptData(MapID, InstanceID, TargetID, 1, 2)--已浇水次数
	local HaveDelete_growthitem = 0
--1.5新增内容////////////////////////////////////////////////up,所有材料▼▼▼▼▼▼
...
--1.5新增内容/////////////////////////////////////////////////down,所有材料▲▲▲▲▲▲▲▲
	...
	if farmer_level > 120 then
		...
		if TalkIndex == -1 then
			if seedowner ~= 0 then
				local MsgID = msg.BeginMsgEvent()
				msg.AddMsgEvent(MsgID, 20, 1329)	-- "###(玩家名)所种的###(植物名),幼苗开始茁壮成长，请多多施肥保证养分！"
				msg.AddMsgEvent(MsgID, 2, seedowner)	--传入玩家名
				msg.AddMsgEvent(MsgID, 21, 4)	-- 选项一
				...
				local MsgID = msg.BeginMsgEvent()
				msg.AddMsgEvent(MsgID, 20, 1332)	-- "珍肴灵树刚刚开始茁壮成长，请多多施肥保证养分！\n它还没有主人，只要您浇过一次水即为它的新主人"
				msg.AddMsgEvent(MsgID, 21, 4)	-- 选项一
				msg.AddMsgEvent(MsgID, 1, 1312)	-- "★施肥"
				msg.AddMsgEvent(MsgID, 21, 5)	-- 选项二
				msg.AddMsgEvent(MsgID, 1, 1303)	-- "离开"
	...
	if 	HaveDelete_growthitem == 1 then
		...
		if growtime == 5 then
			...
			if i ~= 99 then
				local x, y, z = unit.GetPosition(MapID, InstanceID, TargetID)
				local creatureID = map.MapCreateCreature(MapID, InstanceID, 4900413, x, y, z)
				if seedowner == 0 then
				seedowner = RoleID
				end
				cre.SetCreatureScriptData(MapID, InstanceID, creatureID, 1, 1, seedowner)--记录种植出玩家的ID
...
aux.RegisterCreatureEvent(4900412, 7, "ZhenYaoShu_OnTalk")

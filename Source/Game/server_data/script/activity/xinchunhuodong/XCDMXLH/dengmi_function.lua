...
function XinChunDengMi_InitNullTable(RoleID)
	...
	if XinChunDengMi_Questions[RoleID] == nil then
		XinChunDengMi_Questions[RoleID] = {NUM = 0, ID = 0, Quest = {} }  --NUM为将要答的题数（一共20），id为将要答的题号，Quest为表，记录所有答过的题号
	end
...
function DM_GetQuestion(RoleID, DM_CallDeep)
	DM_CallDeep = DM_CallDeep + 1		-- 递归深度+1
	local question
	local index
	...
	if flag == 1 then
		if DM_CallDeep >= 5 then			-- 递归深度超过5的话就直接返回重复的题
			return question
		else
...
function XinChunDengMi_GenerateQuestion(RoleID, TargetID)
	...
	if k == 20 then                                           --这一句很关键，如果玩家在第二十道题目没有选，则可能会无法获得奖励。
		...
		XinChunDengMi_Attend[RoleID] = 1		               -- 已参加过本次活动
	end
	...
	msg.AddMsgEvent(MsgID, 12, XinChunDengMi_Role_Point[RoleID])		-- 得分
